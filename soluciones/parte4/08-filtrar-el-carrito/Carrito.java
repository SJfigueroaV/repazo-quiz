...
public class Carrito {
    ...
    public List<Facturable> itemsDesde(double minimo) {
        List<Facturable> resultado = new ArrayList<>();
        for (Facturable item : items) {
            if (item.calcularImporte() >= minimo) {
                resultado.add(item);
            }
        }
        return resultado;
    ...
    public int quitarMenoresA(double minimo) {
        int antes = items.size();
        items.removeIf(item -> item.calcularImporte() < minimo);
        return antes - items.size();
    }
}
